...
const Single = () => {
  ...
  useEffect(() => {
    const productFromDb = JSON.parse(localStorage.getItem("Meesho-Products"));
    if(productFromDb) {
  ...
  useEffect(() => {
    var user = JSON.parse(localStorage.getItem("MesshoCurrent-user"));
  ...
  function addtocart() {
    // alert("hii")
    if (isUserLoggedin) {
      const user = JSON.parse(localStorage.getItem("Meesho-Users"));
      for (var i = 0; i < user.length; i++) {
        if (user[i].email == CurrentUserEmail) {
          user[i].cart.push(singleproduct);
          localStorage.setItem("Meesho-Users", JSON.stringify(user));
          break;
        }
      }
  ...
  function handleSubmit(e) {
    ...
    e.preventDefault();
    const allPro = JSON.parse(localStorage.getItem('Meesho-Products'))
    for (var i = 0; i < allPro.length; i++) {
        if (allPro[i].id === id) {
            ...
            localStorage.setItem("Meesho-Products", JSON.stringify(allPro));
